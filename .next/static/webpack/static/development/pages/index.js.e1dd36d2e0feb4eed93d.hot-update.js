          ...
          if (re) {
            ...
            if (propStr.length) {
              propStr = propStr.map(function (i) {
                var prop_name,
                    prop_val,
                    reg = /.+/; //handle carriage return

                if (!i.match(reg)) return;
                i = i.match(reg)[0]; //handle extra {

                var a = i.indexOf('{');
                a !== -1 ? i = i.slice(a + 1) : i = i; //handle colons inside property value;

                var notUrl = /:[^\/]/;
                console.log('look out for not url :', i.match(notUrl));
                var re_notUrl = i.match(notUrl);
                if (!re_notUrl) return;
                prop_name = i.substring(0, re_notUrl.index - 1);
                console.log(prop_name, prop_val);
              });
            }
...
//# sourceMappingURL=index.js.e1dd36d2e0feb4eed93d.hot-update.js.map